...
public class HabitList extends Activity implements HabitEntryListener, LoaderCallbacks<Cursor> {
	...
	private void setUpListViewAndAdapter() {
		...
		mAdapter = new SimpleCursorAdapterWithAlternating(this, R.layout.habit_list_item, null, columns, new int[] {
				R.id.textView_habit_list_name, R.id.textView_habit_list_goal }, 0);

		/*
		 * We need to perform some calculations for each line. Ideally I would
		 * do all this in "getView" with a custom adapter, but cursorloader
		 * works too well and I don't know how to use a regular Loader with a
		 * custom adapter (if it's even possible). So I'm intercepting control
		 * when the view is bound, making sure it runs only once for each item
		 * and returning false so that regular binding can occur.
		 */
	...
	private class SimpleCursorAdapterWithAlternating extends SimpleCursorAdapter {

		/*
		 * Wrapper class for simplecursor adapter with the only difference being
		 * that getView returns a view whose background alternates.
		 */

		private int[] colors = new int[] { 0x30ffffff, 0x30808080 };

		public SimpleCursorAdapterWithAlternating(Context context, int layout, Cursor c, String[] from, int[] to, int flags) {
	...
	private void initializeListViewAndSetupCab() {
		...
		listViewHabit.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				/*
				 * Get the Habit name and pass it along. Ideally we should send
				 * the id instead, but this eliminates an additional step for
				 * our small application
				 */

				String habitName = "";
				Cursor c = getContentResolver().query(HabitColumns.CONTENT_URI_HABITS, new String[] { HabitColumns.HABIT_NAME },
						HabitColumns._ID + "=?", new String[] { String.valueOf(id) }, null);
				if (c.moveToFirst()) {
		...
		listViewHabit.setMultiChoiceModeListener(new MultiChoiceModeListener() {
			...
			public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
				...
				case R.id.menu_habit_delete:
					...
					/*
					 * We delete longSelectedId which we set when we first came
					 * in
					 */
					getContentResolver().delete(HabitColumns.CONTENT_URI_HABITS, HabitColumns._ID + "=?",
							new String[] { String.valueOf(longSelectedId) });
					break;
			...
			public void onItemCheckedStateChanged(ActionMode mode, int position, long id, boolean checked) {
				 ...
				 * So inCab will be false the first time we come in and we
				 * assign the id to longSelected which we will be deleting if
				 * the user chooses. If the user selects more stuff after the
				 * long click, longSelected won't change because we've set it to
				 * true.
				 */
				if (!inCab) {
					longSelectedId = id;
